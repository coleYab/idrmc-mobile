...
const DisastersTab = () => {
  ...
  const filteredDisasters = useMemo(() => {
    ...
    return disastersData
      .filter((disaster: any) => {
        ...
        if (statusFilter !== "ALL" && disaster.status?.toUpperCase() !== statusFilter) return false;
        if (typeFilter !== "ALL" && disaster.type?.toUpperCase() !== typeFilter) return false;
        if (severityFilter !== "ALL" && disaster.severity?.toUpperCase() !== severityFilter) return false;
  ...
  }, [searchQuery, statusFilter, typeFilter, severityFilter, sortBy, disastersData]);

  const totalPages = Math.max(1, Math.ceil(filteredDisasters.length / ITEMS_PER_PAGE));
    ...
    <SafeAreaView className="flex-1 bg-background p-5">
      {isLoading ? (
        <View className="flex-1 items-center justify-center">
          ...
          <Text className="mt-4 font-sans-medium text-muted-foreground text-base">Loading disasters...</Text>
        </View>
      ) : (
        <>
          <FlatList
            data={paginatedDisasters}
