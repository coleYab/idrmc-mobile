      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        try {
          const response = await fetch(
            "https://idrmcbkd.onrender.com/api/v1/uploads/image",
            {
              method: "POST",
              body: formData,
              headers: {
      ...
      >
        ...
        <View className="mb-8">
          ...
          <Text className="text-base font-sans-medium text-muted-foreground mt-2 leading-6">
            Please provide the details below to submit a new disaster or
            incident report.
          </Text>
        </View>
        ...
        <View className="auth-form mb-10 gap-y-2">
          ...
          <View className="flex-row gap-4">
            <View className="flex-1 auth-field">
              ...
              >
                {incidentType ? (
                  <Text className="dropdown-value" numberOfLines={1}>
                    {incidentType}
                  </Text>
                ) : (
            ...
            <View className="flex-1 auth-field">
              ...
              <Pressable
                className="dropdown-btn"
                onPress={() =>
                  openDropdown("Select Severity", severityLevels, setSeverity)
                }
              >
                {severity ? (
                  <Text className="dropdown-value" numberOfLines={1}>
            ...
            <View className="flex-row flex-wrap gap-2 mt-4 pt-4 border-t border-border/50">
              {infrastructureDamages.length ? (
                infrastructureDamages.map((damage, idx) => (
                  <View
                    key={idx}
                    className="bg-primary/10 px-3 py-2 rounded-lg flex-row items-center border border-primary/20"
                  >
                    <Text className="text-primary font-sans-medium mr-2">
                      {damage}
                    </Text>
                    {/* <Pressable
      ...
      >
        <View className="dropdown-modal-backdrop">
          ...
          <View className="dropdown-modal-content">
            ...
            <Pressable
              className="mt-6 auth-secondary-button border-transparent"
              onPress={() => setDropdownVisible(false)}
            >
